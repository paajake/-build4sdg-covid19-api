...
class Estimator
{
    private function infectionsByRequestedTime($currently_infected, $period, $duration){
        $factor = null;
        if($period == "months"){
    ...
    private function dollarsInFlight($period, $duration, $avgIncome, $population, $infections){
        $daysFactor = null;
        if($period == "months"){
    ...
    public function covid19ImpactEstimator($data)
    {
        ...
        $infectionsByRequestedTime = $this->infectionsByRequestedTime($currentlyInfected,
            $data["periodType"],
            $data["timeToElapse"]
        );
        $severeInfectionsByRequestedTime = $this->infectionsByRequestedTime($currentlyInfectedSevere,
            $data["periodType"],
            $data["timeToElapse"]
        );
        ...
        $dollarsInFlight = $this->dollarsInFlight( $data["periodType"],
            $data["timeToElapse"],
        ...
        $severeDollarsInFlight = $this->dollarsInFlight( $data["periodType"],
            $data["timeToElapse"],
            $data["region"]["avgDailyIncomeInUSD"],
            $data["region"]["avgDailyIncomePopulation"],
